...
describe("solveUnion — struct discrimination", () => {
  it("discriminates on a common literal 'type' key", () => {
    const r = new KTypeRegistry();
    const a = r.struct([["type", r.literal("a"), false], ["x", r.int(), false]]);
    const b = r.struct([["type", r.literal("b"), false], ["y", r.string(), false]]);
  ...
  it("recurses into a [type, v] sub-discriminator", () => {
    const r = new KTypeRegistry();
    const a1 = r.struct([["type", r.literal("a"), false], ["v", r.literal(1), false], ["x", r.int(), false]]);
    const a2 = r.struct([["type", r.literal("a"), false], ["v", r.literal(2), false], ["y", r.string(), false]]);
    const b = r.struct([["type", r.literal("b"), false], ["v", r.literal(1), false], ["z", r.bool(), false]]);
...
describe("solveUnion — error paths", () => {
  it("rejects a struct union with no discriminator", () => {
    const r = new KTypeRegistry();
    const a = r.struct([["x", r.int(), false], ["z", r.int(), false]]);
    const b = r.struct([["y", r.int(), false], ["w", r.int(), false]]);
  ...
  it("rejects a discriminator that is not present on every member", () => {
    const r = new KTypeRegistry();
    const a = r.struct([["type", r.literal("a"), false], ["x", r.int(), false]]);
    const b = r.struct([["kind", r.literal("b"), false], ["y", r.int(), false]]);
  ...
  it("rejects a sole discriminator that does not uniquely identify all members", () => {
    const r = new KTypeRegistry();
    const a = r.struct([["type", r.literal("same"), false], ["x", r.int(), false]]);
    const b = r.struct([["type", r.literal("same"), false], ["y", r.int(), false]]);
