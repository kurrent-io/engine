...
export default function ListView({ client, list }: { client: KesqClient; list: ListData }) {
  ...
  // a parameterized server query: list.id crosses the wire as an argument,
  // while the rows below render from the local allLists query — one screen,
  // both execution modes
  const stats = useQuery(client, useMemo(() => listStats(list.id), [list.id]));
          ...
          <div className="row hoverable">
            ...
            {stats && (
              <span className="badge">
                {stats.done}/{stats.total} done · server
              </span>
            )}
