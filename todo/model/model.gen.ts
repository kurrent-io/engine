...
export type ConcreteStruct = {};

export type ConcreteTuple = [];

export type List = {id: string, name: string, items: string[], archived: boolean};

export type Item = {id: string, text: string, done: boolean, archived: boolean};

export type NewList = {type: "new-list", id: string, name: string};

export type RenameList = {type: "rename-list", id: string, name: string};

export type ArchiveList = {type: "archive-list", id: string};

export type NewItem = {type: "new-item", id: string, list: string, text: string};
...
export function DecodeConcreteTuple(val: any): ConcreteTuple {
  return val as ConcreteTuple;
}

export function DecodeList(val: any): List {
  return val as List;
}

export function DecodeItem(val: any): Item {
  return val as Item;
}

export function DecodeNewList(val: any): NewList {
  return val as NewList;
}

export function DecodeRenameList(val: any): RenameList {
  return val as RenameList;
}

export function DecodeArchiveList(val: any): ArchiveList {
  return val as ArchiveList;
}

export function DecodeNewItem(val: any): NewItem {
  return val as NewItem;
}

export function DecodeEditItem(val: any): EditItem {
...
export function DecodeArchiveItem(val: any): ArchiveItem {
  return val as ArchiveItem;
}

export function DecodeTodoEvents(val: any): TodoEvents {
  return val as TodoEvents;
}
...
function *queryGet<T>(key: string): QueryGenerator<T> {
  ...
  return readOnly(sv.value) as T
}

function *reducerOld<T>(key: string): Reducer<T> {
  const ans = yield {'old': {[key]: true}};
  const sv = ans.old[key];
  if ('err' in sv) throw sv.err;
  return copyOnWrite(sv.value) as T
}

function *reducerGet<T>(key: string): Reducer<T> {
...
function *reducerSet<T>(key: string, value: T): Reducer<void> {
  const ans = yield {'set': {[key]: value}};
  const sv = ans.set[key];
  if ('err' in sv) throw sv.err;
}
function *reducerDel(key: string): Reducer<void> {
  const ans = yield {'del': {[key]: true}};
  const sv = ans.del[key];
  if ('err' in sv) throw sv.err;
}
function *reducerUpdate<T, R>(key: string, fn: (t: T) => R): Reducer<R> {
  const obj = yield* reducerGet<T>(key);
  const out = fn(obj);
  yield* reducerSet(key, obj);
  return out;
}
export type NoSet<T extends {
  "get": unknown, "old": unknown, "del": unknown, "update": unknown
}> = Pick<T, "get"|"old"|"del"|"update">;

export const TodoQueryContext = {
  get: {
    all_lists: () => queryGet<string[]>(`all_lists`),
    item: (item_id: string) => queryGet<Item>(`item.${item_id}`),
    list: (list_id: string) => queryGet<List>(`list.${list_id}`),
  },
};


export type TodoQX = typeof TodoQueryContext;
export const TodoReducerContext = {
  old: {
    all_lists: () => reducerOld<string[]>(`all_lists`),
    item: (item_id: string) => reducerOld<Item>(`item.${item_id}`),
    list: (list_id: string) => reducerOld<List>(`list.${list_id}`),
  },
  get: {
    all_lists: () => reducerGet<string[]>(`all_lists`),
    item: (item_id: string) => reducerGet<Item>(`item.${item_id}`),
    list: (list_id: string) => reducerGet<List>(`list.${list_id}`),
  },
  set: {
    all_lists: (value: string[]) => reducerSet(`all_lists`, value),
    item: (item_id: string, value: Item) => reducerSet(`item.${item_id}`, value),
    list: (list_id: string, value: List) => reducerSet(`list.${list_id}`, value),
  },
  del: {
    item: (item_id: string) => reducerDel(`item.${item_id}`),
    list: (list_id: string) => reducerDel(`list.${list_id}`),
  },
  update: {
    all_lists: <R>(fn: (value: string[]) => R) => reducerUpdate(`all_lists`, fn),
    item: <R>(item_id: string, fn: (value: Item) => R) => reducerUpdate(`item.${item_id}`, fn),
    list: <R>(list_id: string, fn: (value: List) => R) => reducerUpdate(`list.${list_id}`, fn),
  },
};
